...
const Card: FC<IProps> = ({ task, id}) => {
    ...
    return (
        <div className={css.Card} ref={setNodeRef} {...listeners} { ...attributes } style={{ transform: CSS.Transform.toString(transform), transition: transition }}>
            <div className={css.cardContent}>
            <h2>{name}</h2>
            <p>{description}</p>
            </div>
            <div className={css.cardActions}>
            <button type={'button'} onClick={() => editCard(task)}><FaRegEdit /></button>
            <button type={'button'} onClick={() => deleteCard(_id)}><RiDeleteBin6Line /></button>
            </div>
        </div>
    );
};
