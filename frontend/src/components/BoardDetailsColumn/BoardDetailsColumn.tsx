...
const BoardDetailsColumn: FC<IProps> = ({title, items}) => {
    ...
    return (
        <div className={css.section}>
            <h1>{title}</h1>
            <SortableContext id={title} items={itemIds} strategy={verticalListSortingStrategy}>
                <div className={css.cards} ref={setNodeRef}>
                    {
                      items &&  items.map( (item) => <Card key={item?._id} id={item?._id} task={item}/> )
                    }

            {title === ECardStatus.ToDo && <div className={css.addAction} onClick={() => dispatch(modalActions.setShowModal('card'))}><RiAddLargeFill /></div>}
                </div>
            </SortableContext>
        </div>
    );
